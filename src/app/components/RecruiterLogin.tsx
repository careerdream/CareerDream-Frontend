...
import { Mail, Lock, Building2, Eye, EyeOff, AlertCircle, Loader2, CheckCircle2 } from 'lucide-react';
...
export function RecruiterLogin() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    setError('');
    setSuccess('');
    if (!email) { setError('Please enter your business email address.'); return; }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-center gap-3 mb-6">
            ...
            <h1 className="text-2xl font-bold">{view === 'login' ? 'Recruiter Access' : 'Reset Password'}</h1>
          </div>
          <p className="text-center text-muted-foreground mb-6">
            {view === 'login' ? 'Sign in to post jobs and manage applications' : 'Enter your business email to receive a password reset link'}
          </p>
          ...
          {success && (
            <div className="mb-4 p-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900 flex gap-2">
          ...
          {view === 'login' ? (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 rounded-lg bg-gradient-to-r from-primary to-accent text-white font-semibold hover:shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2 mt-2"
              >
                {loading ? (
                  <>
